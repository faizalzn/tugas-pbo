/**
 * This SQL script creates and manages the `perpustakaan` database, which includes tables for books, book borrowing and returns, fines, and other related entities.
 * 
 * The main tables in this database are:
 * - `tb_buku`: Stores information about books, including title, author, publisher, publication year, quantity, registration date, shelf location, city of publication, book type, ISBN/ISSN, volume, and edition.
 * - `tb_denda`: Stores information about fines, including the fine description, fine type, and the associated book return.
 * - `tb_detail_peminjaman`: Stores information about book borrowing, including the book ID, the borrowing ID, and the book status (borrowed or returned).
 * - `tb_detail_pengembalian`: Stores information about book returns, including the book ID and the associated return ID.
 * - `tb_jenis_denda`: Stores information about different types of fines, including the fine type and the associated fine amount.
 * - `tb_kota_terbit`: Stores information about the cities where books are published.
 * - `tb_member`: Stores information about library members, including name, gender, date of birth, phone number, email, address, and registration date.
 * - `tb_pegawai`: Stores information about library staff, including name, gender, phone number, password, and username.
 * - `tb_peminjaman`: Stores information about book borrowing, including the staff ID, member ID, borrowing date, and return date.
 * - `tb_penerbit`: Stores information about book publishers.
 * - `tb_penulis`: Stores information about book authors.
 * - `tb_rak`: Stores information about book shelves, including the shelf number.
 * - `tb_tag`: Stores information about book tags, including the tag name.
 * - `tb_tag_buku`: Stores the association between books and their tags.
 */
/*
...
USE `/*Table structure for table `tb_buku` */
...
PRIMARY KEY (`id`),
  ...
  KEY `penerbit_id` (`penerbit_id`),
  KEY `rak_id` (`rak_id`),
  KEY `kota_terbit_id` (`kota_terbi

  KEY `penulis_id` (`penulis_id`),
  KEY `penerbit_id` (`penerbit_id`),
  KEY `rak_id` (`rak_id`),
  KEY `kota_terbit_id` (`kota
...
=InnoDB AUTO_INCREMENT=14 DEFAULT CHARSET=utf8mb4;
